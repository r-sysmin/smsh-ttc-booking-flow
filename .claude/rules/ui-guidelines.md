...
- **Sidebar footer is reserved for settings and misc** — user profile, preferences, the leave affordance
- **The account menu holds the leave affordance, and `useIsDemo()` decides which one.**
  Read demo-ness from `src/lib/demo.ts` only — never re-derive it from the pathname or a
  route prefix:
  ...
  - public `/demo/*` → "Exit demo" with a close icon (`IconX`), a plain navigation to
    ...
    but leaving a demo visitor with no way out is also wrong.
  This app uses the top-bar layout (`workspace-layout-02`), which cannot mount a shadcn
  `SidebarAccountFooter` (that needs `SidebarProvider`), so the affordance is the last item
  in the account dropdown in `src/pages/workspace/components/workspace-top-bar-02.tsx`.
...
8. ❌ Hand-rolled "Continue with Google/Apple" buttons (Tabler glyph in an outline `Button`). ✅ `<SocialAuthButtons>` from `@/components/base/social-auth-buttons`, never restyled with the theme color
9. ❌ A demo shell whose only leave affordance says "Log out". ✅ "Exit demo" → `EXIT_DEMO_ROUTE`, chosen by `useIsDemo()`
