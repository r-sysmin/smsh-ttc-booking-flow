...
- `src/components/protected-route.tsx` — redirects to `/auth` when `!user && !loading`, preserves `from` in location state
- Wrap `/*` app routes (not `/demo/*`) in `ProtectedRoute` in `App.tsx`
- Account menu: sign-out calls `signOut()` (which clears the React Query cache)
- `/auth/callback` (`src/pages/auth/callback.tsx`) — every OAuth and email-confirmation link returns here; without the route SSO 404s

**Four Don'ts that are shipped code, not suggestions** (see `docs/design/auth.md`):

1. Don't hand-roll "Continue with Google/Apple" buttons. The ONE brand-compliant set is `<SocialAuthButtons>` (`@/components/base/social-auth-buttons`). Never restyle it with the theme color, and pass only the providers this project has configured (`["google"]`).
2. Don't call `supabase.auth.signInWithOAuth` — the Google/Apple providers here are secretless managed providers, so a direct GoTrue call fails with "missing OAuth secret". OAuth goes through the Lovable broker inside `<SocialAuthButtons>`. Never edit `src/integrations/lovable/` — it is generated.
3. Don't redirect OAuth or email confirmation to a bare `window.location.origin` — always `${window.location.origin}/auth/callback`, which lands on `DEFAULT_AUTHED_ROUTE` (`src/lib/auth/constants.ts`). Password RECOVERY is the one exception: it returns to `/auth`, so the user can choose a new password instead of being silently signed in.
4. Don't leave a demo shell with no way out. On `/demo/*` the leave affordance is "Exit demo" → `EXIT_DEMO_ROUTE`, decided by `useIsDemo()` (`src/lib/demo.ts`) and nothing else.
